...
- **Session name in stats DB.** The `session_name` column (schema v4) is now written to every `tool_calls` row. Read-only opens of older databases (pre-v4) fall back to an empty string — existing history is preserved. The `Filter` struct gains a `SessionName` field for narrowing queries by name. The TUI popup detail row shows `NAME  id  ● current` and the session right-panel shows a dedicated `Name` row above the session ID.
...
- **Stats `OpenReadOnly` reports schema version clearly.** Old databases (schema v1/v2) opened read-only by `plumb status` or the TUI now return a clear "database schema is version N, need version 3 — open with a writer to migrate" diagnostic instead of a raw SQL error about a missing column.

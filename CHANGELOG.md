...
### Fixed
- **`plumb stats` no longer shows stale rows after a project move.** Per-project stats DBs can move with the checkout, but older rows may still carry the previous absolute workspace path in their metadata. `plumb stats` now filters rows to the requested workspace path instead of showing every row in the DB. Stats inserts now return errors up to the daemon, which logs failed writes instead of silently dropping them.

...
| `internal/topology/` | SQLite/FTS5 semantic graph; background indexer; Go AST, tree-sitter Python/Rust/Zig/Kotlin/Swift/Java, and TypeScript/JS regex extractors; search + BFS explore/impact/affected/routes |
| `internal/topology/extractors/golang/` | Go extractor using `go/parser`+`go/ast`; no CGo |
| `internal/topology/extractors/treesitter/` | gotreesitter-backed extractors (pure-Go runtime, no CGo) — **Python, Rust, Zig, Kotlin, Swift, Java live** (Java pairs the structural Map with the jdtls LSP). Only **TS/TSX** is deferred: gotreesitter v0.19.1 ships its external scanner but not the external **lex-states** table, so real-world TS (typed arrow params) parses with cascading errors; Kotlin and plain JavaScript are unaffected — see `docs/internal/treesitter-plan.md`. Imports the gotreesitter `grammars` package, which embeds all 206 grammar blobs plus the pure-Go runtime (~+26 MB; ~46 MB binary total). |

...
| `internal/lsp/adapters/{gopls,pyright,jdtls,rust,swift,zig,typescript}/` | Go, Python, Java, Rust, Swift (validated), Zig + TypeScript/JS (experimental — unit-tested, integration pending binary install). All non-Go/Python adapters are opt-in via `[lsp.<lang>] enabled = true`. |

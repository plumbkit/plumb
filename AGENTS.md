...
fsync = true                  # fsync-before-ack: fsync the staged temp file before the atomic rename and the parent directory after it, so an acknowledged write survives a hard crash; set false to skip both fsyncs (benchmarks, exotic filesystems). DAEMON-GLOBAL — it gates write primitives shared by every session, so it is read from global config only and a per-project override is ignored
...
`commit_trailer` (env `PLUMB_GIT_COMMIT_TRAILER`) opts into **commit attribution in the git history**: every plumb-mediated commit is stamped with a `Plumb-Session: <session-name>` trailer, so `git log --format=%(trailers)` attributes a commit to the agent session that authored it. **Requires git ≥ 2.32** — `git commit --trailer` was added in that release, plumb runs no version probe, and turning this on against an older git fails every commit issued through the tool with `error: unknown option 'trailer'`. Off by default — the trailer is per-commit noise, and attribution is always queryable without it: `workspace_sessions` renders recent commits per session (short SHA, subject, repository) from its stats-backed recent-writes feed.
...
| Claude Code, user scope | `plumb setup claude-code` | `~/.claude.json` + `~/.claude/skills/` (skills via `plumb skills sync`) |
| Claude Code, project scope | `plumb setup claude-code --project` | `.mcp.json` in the current directory + `~/.claude/skills/` (skills via `plumb skills sync`) |
| Codex | `plumb setup codex` | `$CODEX_HOME/config.toml`, or `~/.codex/config.toml` when unset, + `$CODEX_HOME/skills/` (skills via `plumb skills sync`) |
...
| Kimi Code | `plumb setup kimi-code` (`--lean` writes a client-side tool allowlist) | `$KIMI_CODE_HOME/mcp.json`, or `~/.kimi-code/mcp.json` when unset (`mcpServers` key; Kimi Desktop reads the same file, so one registration covers both) + `$KIMI_CODE_HOME/skills/` (skills via `plumb skills sync`) |
...
**`plumb setup kimi-code --lean` — the client-side allowlist** (`internal/cli/setup_kimi.go`). Kimi Code is schema-discovery-only, so plumb's own lean profile cannot help it (a hidden tool would be unreachable, not merely undisplayed) and `auto` correctly serves it **full**. The saving is taken on the client instead: `--lean` writes `tools.LeanToolNames()` — the sorted union of `LeanTools` and `BootstrapTools`, and the single source of truth for any client-side allowlist — into the `enabledTools` key of the plumb entry in Kimi's own `mcp.json`, so Kimi loads ~21 schemas instead of all 57. Three contract points: the idempotence predicate is **lean-aware** (without that, `mergeServerEntry`'s "already points at this binary" short-circuit would make `--lean` a silent no-op on every already-registered machine); a later **bare** `plumb setup kimi-code` (and `--all`) **preserves** the key, the same merge-onto-existing contract that keeps Codex's approval tables, while `--lean` over a hand-edited list **replaces** it; and the list is a **snapshot** — re-run `--lean` after an upgrade to refresh it. There is deliberately no `--full` un-set flag: removing the key is a one-line manual edit. `plumb doctor` prints an informational line (never a warning — a full registration is a valid default) when Kimi registers plumb with no `enabledTools` key. When the key **is** there it is graded on **content, not shape** (`gradeToolAllowlist`, `internal/cli/doctor_allowlist.go`): a list naming at least one registered tool passes; a list naming none of them is a **warning with a fix** (`["", ""]`, `[1, 2, 3]` and `["not_a_plumb_tool"]` are all well-formed non-empty JSON arrays that leave Kimi with zero plumb tools, so grading on shape alone reported a dead integration as healthy); a list that reads as plumb's own aged snapshot of the lean set is an informational drift hint, not a "!". A value that cannot be an allowlist at all is also a warning with a fix, but the three such shapes are **worded separately** rather than sharing one sentence, because only one of them is a behaviour plumb can actually observe: `[]` (or a list holding no tool name) definitely loads zero tools; `null` is most likely read as no allowlist at all — the *full* surface — so doctor says it cannot verify which way Kimi takes it and offers deleting the key as the unambiguous fix; a wrong-typed value is one plumb cannot predict the parse of (ignored, coerced, or the whole server entry rejected). The value is named in **JSON's** vocabulary (`a number`, `an object`), never Go's `%T`.

Two bulk flags on the bare `plumb setup` command (`runSetupAll`, `internal/cli/setup_clients.go`): `--repair` **repoints** every already-registered client at the current binary — the idempotent repair `plumb doctor` recommends after the binary moves or is rebuilt; it never adds plumb to a client that lacked it. `--all` additionally **registers** plumb in installed-but-unregistered clients (config file present, no plumb entry) — the one-shot first-time setup — but never fabricates a config for a client with no config file (an absent config is indistinguishable from an uninstalled client). `--install-missing` survives one release as a hidden, deprecated alias for `--all`. The one exception to the no-fabrication rule is Kimi Code: its `mcp.json` only exists once an MCP server is configured, so its target carries an optional `installedFn` that detects the install via the data dir (`$KIMI_CODE_HOME`, or `~/.kimi-code`) — the bulk flags then treat it as installed-but-unregistered (and `--all` creates the config), and `plumb doctor` reports "installed, but plumb is not registered" instead of "not installed". Any of the three flags triggers the bulk run; `refreshClientAt` classifies each config path as `not installed` / `not registered` / `already current` / `registered` / `updated`, and a `--repair` run that finds unregistered clients prints a hint pointing at `--all`.

`plumb skills` owns the skill surface, now that setup is config-only: `plumb skills sync` installs seven idempotent user-scoped skills for the clients that read `SKILL.md` — `plumb-explore` (the discovery ladder, `workspace_search` down to a bounded `read_file`), `plumb-refactor` (semantic rename, atomic cross-file edits), `plumb-testing` (`topology_affected` for which tests to run, `run_task` to run them), `plumb-minimal-change` (prove reuse and minimality with plumb evidence before writing non-trivial code), `plumb-memory` (search the per-workspace memories before re-deriving, write one when the knowledge outlives the task), `plumb-diagnose` (read the refusal, get compile truth, tell "broken" from "not ready"), and `plumb-git` (the tiered git policy, and the narrower plumb tool to prefer over a destructive git command). Bare `plumb skills` is a read-only per-client, per-skill status table (installed / missing / stale, with a `not registered` marker for skill-capable clients whose config lacks plumb); `plumb skills sync [client]` is the only writer, installing or refreshing them into every registered skill-capable client, or just the named one.

**Which clients, and where.** Skill capability is per-client data (`setupTarget.skillsDirFn`, `internal/cli/setup_skills.go`), verified against a live install rather than inferred: Claude Code (`~/.claude/skills/`), Codex (`$CODEX_HOME/skills/`, else `~/.codex/skills/`), and Kimi Code (`$KIMI_CODE_HOME/skills/`, else `~/.kimi-code/skills/`). Every other client's `skillsDirFn` is nil and stays nil until someone verifies a real directory — writing files into a guessed path is worse than not writing them, so `TestSkillCapableClients_ArePinned` makes the set a deliberate edit. Registration never writes skill files: a named `plumb setup <client>` prints a drift hint ("Skills missing/outdated — run `plumb skills sync <client>`") when it detects missing or stale skills, `plumb doctor` grades the same drift as an informational line (never a warning), and re-running `plumb skills sync` after an upgrade is how new skill content lands.

Clients with no skill channel get the same routing as a **condensed `session_start` block** (`writeGenericGuidance`, `internal/tools/session_start_guidance.go`) — one authored source, two render targets. The skills are the canonical, expanded home for multi-tool workflows; tool descriptions and `session_start` guidance point at them rather than restating them. Seven is a stated ceiling, not a drift: the reasoning lives on `skillsFS` in `internal/cli/skills.go`, and an eighth skill has to argue against it.

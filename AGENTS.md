...
| `internal/lsp/adapters/{gopls,pyright,jdtls}/` | Go (validated), Python (validated), Java (validated; experimental, `[lsp.java] enabled = true`) adapters |
| `internal/topology/` | SQLite/FTS5 semantic graph; background indexer; Go AST, tree-sitter Python/Rust/Zig/Kotlin/Swift/Java/Bash/HCL/SQL/Dockerfile/TOML/YAML, and TS/JS regex extractors; search + BFS explore/impact/affected/routes |
| `internal/topology/extractors/golang/` | Go extractor (`go/parser`+`go/ast`; no CGo) |
| `internal/topology/extractors/treesitter/` | gotreesitter extractors (pure-Go, no CGo): **Python, Rust, Zig, Kotlin, Swift, Java, Bash, HCL, SQL, Dockerfile, TOML, YAML live** (the config/IaC/query grammars — Bash, HCL, SQL, Dockerfile, TOML, YAML — extract their named declarations; TOML/YAML index the config-key tree via containment edges; Dockerfile matches by basename via the extensionless-file matcher in `findExtractor`). TS/TSX deferred — gotreesitter v0.19.1 ships the external scanner but not the lex-states table, so typed TS parses with cascading errors (plain JS/Kotlin unaffected). Embeds the `grammars` package (206 blobs + runtime, ~+26 MB; ~46 MB binary). See `docs/internal/treesitter-plan.md`. |
| `internal/topology/extractors/typescript/` | TS/JS regex extractor; no CGo |

...
| Kimi Code | `plumb setup kimi-code` (`--lean` writes a client-side tool allowlist) | `$KIMI_CODE_HOME/mcp.json`, or `~/.kimi-code/mcp.json` when unset (`mcpServers` key; Kimi Desktop reads the same file, so one registration covers both) |

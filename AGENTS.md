...
Controls **which tools appear in `tools/list`**, to spare clients that already have native filesystem tools from paying for ~33 commodity duplicates. `auto` (default) resolves to **lean** for a recognised CLI agent that reads and searches files natively *and* can invoke a tool it was never advertised (Codex, Gemini CLI) and **full** for Claude Desktop, any unrecognised client, and any schema-discovery-only client (Claude Code — see below); `lean`/`full` force it. Project-overridable and `PLUMB_TOOLS_PROFILE`-overridable; resolution precedence is per-client override → `profile` → auto.
...
- **Cross-agent sharing (`[collab]`, opt-in):** `share_intent` broadcasts what you are working on (optionally scoped to `path_globs`); `leave_note` leaves a message for a named peer session or for `next` (whoever attaches next). Both are advisory (never block a write), secret-scrubbed, per-workspace, and rendered to peers as unverified claims. Delivered by polling only (`workspace_sessions`, `session_start`) plus an intent-aware write hint. Gated on `[collab] intents` / `mailbox` (default off); rows expire per `intent_ttl_minutes` in `<workspace>/.plumb/collab.db` (gitignored). See the `[collab]` config section.

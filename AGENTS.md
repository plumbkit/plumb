...
- **Cross-agent sharing (`[collab]`):** `share_intent` declares work; `leave_note` + `check_messages` form a request/reply mailbox; `share_findings` creates a durable generated memory. All are advisory, secret-scrubbed, and labelled agent-authored. **Mailbox:** notes arrive exactly once on successful tool results, `session_start`, or a blocking `check_messages({wait_seconds})` that reports elapsed time and clamps. Start without `conversation_id`; reply by quoting it. An omitted in-thread `to` resolves the one other participant or refuses, never `next`. Threads have no count ceiling. Sender and recipient get exact UTF-8 truncation counts under `chat_budget_bytes` plus a follow-up/file remedy. `workspace_sessions`, TUI, and Web show delivery state and volume for humans, never as enforcement. Delivery is polling — silence is not refusal. Gated on `[collab] intents` (off) / `mailbox` (**on**, same-workspace) / `cross_project` (off, recipient opt-in) / `knowledge_handoff` (off); rows expire per `intent_ttl_minutes`. See `docs/configuration.md`.

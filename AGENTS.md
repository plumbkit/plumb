...
| `internal/topology/extractors/treesitter/` | gotreesitter-backed extractors (pure-Go runtime, no CGo) — **Python, Rust, Zig live**. TS/JS and Kotlin are deferred: gotreesitter v0.19.1 ships their external scanners but not the external **lex-states** tables, so real-world TS (typed arrow params) and Kotlin (class/interface bodies) parse with cascading errors — see `docs/internal/treesitter-plan.md`. Imports the gotreesitter `grammars` package, which embeds all 206 grammar blobs plus the pure-Go runtime (~+26 MB; ~46 MB binary total). |

...
| Codex | `plumb setup codex` | `$CODEX_HOME/config.toml`, or `~/.codex/config.toml` when unset, + `$CODEX_HOME/skills/` (skill files) |
...
| Kimi Code | `plumb setup kimi-code` (`--lean` writes a client-side tool allowlist) | `$KIMI_CODE_HOME/mcp.json`, or `~/.kimi-code/mcp.json` when unset (`mcpServers` key; Kimi Desktop reads the same file, so one registration covers both) + `$KIMI_CODE_HOME/skills/` (skill files) |

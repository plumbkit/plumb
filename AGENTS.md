...
| `internal/lsp/adapters/{gopls,pyright,jdtls,rust,swift}/` | Go (validated), Python (validated), Java (validated; experimental, `[lsp.java] enabled = true`), Rust (validated; opt-in, `[lsp.rust] enabled = true`), Swift (validated; opt-in, `[lsp.swift] enabled = true`) adapters |

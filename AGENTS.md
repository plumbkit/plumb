...
| `internal/topology/` | SQLite/FTS5 semantic graph; background indexer; Go AST, tree-sitter Python/Rust/Zig/Kotlin/Swift/Java/Bash/HCL/SQL/Dockerfile/TOML, and TS/JS regex extractors; search + BFS explore/impact/affected/routes |
| `internal/topology/extractors/golang/` | Go extractor (`go/parser`+`go/ast`; no CGo) |
| `internal/topology/extractors/treesitter/` | gotreesitter extractors (pure-Go, no CGo): **Python, Rust, Zig, Kotlin, Swift, Java, Bash, HCL, SQL, Dockerfile, TOML live** (the config/IaC/query grammars — Bash, HCL, SQL, Dockerfile, TOML — extract their top-level named declarations; Dockerfile matches by basename via the extensionless-file matcher in `findExtractor`). TS/TSX deferred — gotreesitter v0.19.1 ships the external scanner but not the lex-states table, so typed TS parses with cascading errors (plain JS/Kotlin unaffected). Embeds the `grammars` package (206 blobs + runtime, ~+26 MB; ~46 MB binary). See `docs/internal/treesitter-plan.md`. |

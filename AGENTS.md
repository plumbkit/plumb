...
**Single-workspace-per-connection contract (0.8.19+).** Once a connection has attached a workspace, every path-bearing tool — file/list/search/write/git/memory/LSP-URI/topology-status — refuses paths outside that root with a `workspace boundary violation` error. `rename_symbol` also boundary-checks each output URI in the language server's `WorkspaceEdit` before applying. To work on a different project, start a new MCP connection (Claude Desktop and similar clients may reuse one `plumb serve` across chats, so a fresh chat does **not** guarantee a fresh connection; the boundary makes reuse safe rather than relying on client behaviour). A connection that hits a violation has its session marked `Health: blocked` for the TUI dashboard, but legitimate in-workspace calls keep working — the per-call rejection is the contract, the sticky health flag is observability. `git`'s `repo` argument defaults to the pinned workspace when omitted (0.8.20+), so a bare `git status` runs against the session's project rather than the daemon's cwd.

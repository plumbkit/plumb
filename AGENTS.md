...
| `internal/topology/` | SQLite/FTS5 semantic graph; background indexer; Go AST, gotreesitter Python/JavaScript/Rust/Zig/Kotlin/Swift/Java/Bash/HCL/SQL/Dockerfile/TOML/YAML/Markdown/HTML, and canonical-tree-sitter-via-WASM TypeScript/TSX/JSX (`wasmts`); search + BFS explore/impact/affected/routes |
| `internal/topology/extractors/golang/` | Go extractor (`go/parser`+`go/ast`; no CGo) |
| `internal/topology/extractors/treesitter/` | gotreesitter extractors (pure-Go, no CGo): **Python, JavaScript, Rust, Zig, Kotlin, Swift, Java, Bash, HCL, SQL, Dockerfile, TOML, YAML, Markdown live** (the config/IaC/markup grammars extract their named declarations — Bash functions, HCL/SQL/Dockerfile/TOML/YAML config, Markdown headings; TOML/YAML/Markdown index the nesting tree via containment edges; Dockerfile matches by basename via the extensionless-file matcher in `findExtractor`). **JavaScript (`.js`/`.mjs`/`.cjs`)** is on gotreesitter here; **TypeScript (`.ts`) and TSX/JSX (`.tsx`/`.jsx`) moved to the `wasmts` package** (canonical tree-sitter via WASM — gotreesitter cascades on typed-arrow TSX in every version, so the gotreesitter TS extractor and its `ts_lex_states.go` tables were removed). Embeds the `grammars` package (206 blobs + runtime, ~+26 MB). gotreesitter pinned at v0.20.1. See `docs/internal/treesitter-plan.md`. |
| `internal/topology/extractors/typescript/` | Legacy regex TS/JS extractor; no CGo. Retained only as the `wasmts` init-failure fallback — no longer registered as a primary extractor. |
| `internal/topology/extractors/wasmts/` | TypeScript (`.ts`) + TSX/JSX (`.tsx`/`.jsx`) via the **canonical** `tree-sitter-typescript` grammar compiled to WASM and driven by wazero (pure-Go, `CGO_ENABLED=0`). Parses typed-arrow TSX cleanly where gotreesitter cascades. Embeds `ts.wasm` (~2.9 MB); regenerate from vendored C in `csrc/` via `make ts-wasm` (needs Zig; building/running plumb needs only Go + wazero). |
...
- Sections (opened with `/`): `Dashboard`, `Sessions`, `Memory`, `Logs`, `Settings` (indices 0–4). Sessions (index 1, default) is a two-panel layout; Logs (index 3) tails `daemon.log`; Settings (index 4) is a two-pane editor (`internal/tui/model_settings.go`): a left **Scope** column (Global + each workspace) and a right rows pane with **General** / **LSP** tabs. `tab`/`shift+tab` cycle Scope → General → LSP → Scope; `esc` steps focus back to the home panel (Scope here, Sessions/Workspaces elsewhere) rather than opening the section menu. `[`/`]` resize the focused column.
- Settings persistence is scope-aware: Global rows write the global config (`config.Save`), workspace rows write a sparse override to that project's `.plumb/config.toml`. Each row carries a reload-tier numeral — `¹` live / `²` next session / `³` restart — and, in a workspace scope, a superscript `⁴` override / `⁵` inherited mark; only **Theme** and **Log level** apply live (the latter via the control socket in `m.ctrlPath`). `ctrl+t` opens the theme picker from any section.
- List-valued and per-language `[lsp.<lang>]` rows open shared pop-up editors (`editorModalBox` in `model_settings_listeditor.go`, plus `model_settings_texteditor.go` for single strings); list editors auto-save on close, `enter` edits the selected entry. List markers use a muted `∙` bullet app-wide (`❯` when selected).

...
- **No globals** except package-level style vars in `internal/tui/styles.go`, the `pathLocks` map in `internal/tools/file_write_helpers.go`, and `mutationRunLock` in `internal/tools/mutationtest.go` (both process-global by design: they protect the machine, not a session).

...
- **Cross-agent sharing (`[collab]`):** `share_intent` declares work; `leave_note` + `check_messages` form a request/reply mailbox; `share_findings` creates a durable generated memory. All are advisory, secret-scrubbed, and labelled agent-authored. **Mailbox:** notes are atomically claimed at most once across successful tool results, `session_start`, and blocking `check_messages({wait_seconds})`; a transport failure after claim can still lose one, so end-to-end exactly-once is not promised. The wait reports elapsed time and clamps. Start without `conversation_id`; reply by quoting it. An in-thread reply binds the other active participant by stable session ID (following renames) or refuses, never `next` or a reused display name. Threads have no count ceiling. The existing `chat_budget_bytes` bounds both durable body storage and delivery; sender and recipient get exact UTF-8 truncation counts plus a follow-up/file remedy. `workspace_sessions`, TUI, and Web show delivery state and volume for humans, never as enforcement. Delivery is polling — silence is not refusal. Gated on `[collab] intents` (off) / `mailbox` (**on**, same-workspace) / `cross_project` (off, recipient opt-in) / `knowledge_handoff` (off); rows expire per `intent_ttl_minutes`. See `docs/configuration.md`.

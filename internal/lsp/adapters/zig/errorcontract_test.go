...
// TestErrorContract pins the "zls <label>: <cause>" strings this adapter
...
func TestErrorContract(t *testing.T) {
	doc := filepath.Join(t.TempDir(), "main.zig")
	if err := os.WriteFile(doc, []byte("pub fn main() void {}\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	conformance.RunErrorContract(t,
		func(c jsonrpc.Caller) lsp.Client { return zig.New(c) },
		zig.DefaultInitParams, "zls", paths.PathToURI(doc))
}

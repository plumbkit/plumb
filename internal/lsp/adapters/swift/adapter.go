...
// "sourcekit-lsp <label>: <cause>". This package overrides the ones that need
// the document open first (sourcekit-lsp replies -32001 "No language service
// for <uri> found" otherwise) and Definition, whose reply may be a bare single
// Location.
//
...
// Rename performs a workspace-wide rename. It opens the document first, like
// Definition/References/PrepareRename: sourcekit-lsp answers a rename only for a
// document opened via didOpen, and a caller may invoke Rename without a
// preceding PrepareRename, so without this the server would reply -32001 "No
// language service found". The zls adapter deliberately does not.
func (a *Adapter) Rename(ctx context.Context, params protocol.RenameParams) (*protocol.WorkspaceEdit, error) {
	if err := a.open.Ensure(ctx, params.TextDocument.URI); err != nil {
		return nil, err
	}
	return a.Adapter.Rename(ctx, params)
}

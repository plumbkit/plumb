...
var initResult = protocol.InitializeResult{
	...
	ServerInfo: &protocol.ServerInfo{Name: "typescript-language-server", Version: "4.0.0"},
}

// newAdapter sets up a MockCaller with sensible defaults and returns the adapter.
func newAdapter(t *testing.T) (*ts.Adapter, *jsonrpc.MockCaller) {
	t.Helper()
...
func TestAdapter_Initialize(t *testing.T) {
	...
	result, err := ad.Initialize(ctx, ts.DefaultInitParams("file:///project"))
	if err != nil {
	...
	if result.ServerInfo == nil || result.ServerInfo.Name != "typescript-language-server" {
...
func TestAdapter_Initialized(t *testing.T) {
	...
	if _, err := ad.Initialize(ctx, ts.DefaultInitParams("file:///project")); err != nil {
		t.Fatal(err)
	}
...
func TestAdapter_DidChangeWatchedFiles(t *testing.T) {
	...
	err := ad.DidChangeWatchedFiles(ctx, protocol.DidChangeWatchedFilesParams{
		Changes: []protocol.FileEvent{
			{URI: "file:///p/src/index.ts", Type: protocol.FileChanged},
			{URI: "file:///p/src/new.ts", Type: protocol.FileCreated},
		},
	})
	if err != nil {
...
func TestAdapter_DidOpenDidClose(t *testing.T) {
	...
	if err := ad.DidOpen(ctx, protocol.DidOpenTextDocumentParams{
		TextDocument: protocol.TextDocumentItem{
			URI: "file:///p/src/index.ts", LanguageID: "typescript", Version: 1, Text: "const x = 1;\n",
		},
	}); err != nil {
		t.Fatalf("DidOpen: %v", err)
	}
	if err := ad.DidClose(ctx, protocol.DidCloseTextDocumentParams{
		TextDocument: protocol.TextDocumentIdentifier{URI: "file:///p/src/index.ts"},
	}); err != nil {
...
func TestAdapter_WorkspaceSymbols(t *testing.T) {
	...
	expected := []protocol.SymbolInformation{
		{Name: "Greeter", Kind: protocol.SKClass, Location: protocol.Location{URI: "file:///p/src/index.ts"}},
	}
...
func TestAdapter_Definition(t *testing.T) {
	...
	expected := []protocol.Location{
		{URI: "file:///p/src/index.ts", Range: protocol.Range{Start: protocol.Position{Line: 3}}},
	}
...
func TestAdapter_References(t *testing.T) {
	...
	expected := []protocol.Location{
		{URI: "file:///p/src/index.ts", Range: protocol.Range{Start: protocol.Position{Line: 10}}},
		{URI: "file:///p/src/index.ts", Range: protocol.Range{Start: protocol.Position{Line: 14}}},
	}
...
func TestAdapter_Hover(t *testing.T) {
	...
	expected := protocol.Hover{
		Contents: protocol.MarkupContent{Kind: "markdown", Value: "```typescript\nclass Greeter\n```"},
	}
...
func TestAdapter_Rename(t *testing.T) {
	...
	if len(edit.Changes["file:///p/src/index.ts"]) != 1 {

...
func TestOpenCreatesCurrentGlobalSchema(t *testing.T) {
	...
	for _, col := range []string{"session_id", "session_name", "workspace", "input_json", "output_text", "client_name", "client_version", "tokens_saved", "savings_model_version", "capability_tokens", "efficiency_tokens", "purpose", "error_kind", "error_retryable", "remediation_class", "logical_agent"} {

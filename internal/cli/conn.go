...
func (s *connSession) registerAllTools(srv *mcp.Server, daemonStartedAt time.Time) {
	...
	srv.Register(tools.NewReadSymbol(s.sessionProxy, s.sessionCache, s.ttl, lspTimeout, s.readTracker))
	...
	srv.Register(tools.NewReplaceSymbolBody(s.sessionProxy, lspTimeout))

...
func TestScan_NopWhenEmpty(t *testing.T) {
	ws := initWorkspace(t)
	Scan(ws, time.Now().Add(time.Hour), confinedTo(ws)) // must not panic or error
}

// TestScan_EmptyWorkspace is a no-op on empty workspace.
func TestScan_EmptyWorkspace(t *testing.T) {
	Scan("", time.Now(), nil) // must not panic
}
...
func TestScan_MissingManifest(t *testing.T) {
	...
	Scan(ws, time.Now().Add(time.Hour), confinedTo(ws)) // should log an error and continue
...
func TestScan_CorruptManifest(t *testing.T) {
	...
	Scan(ws, time.Now().Add(time.Hour), confinedTo(ws)) // must not panic
}

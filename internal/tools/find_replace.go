	...
	for range workers {
		wg.Go(func() {
			for path := range paths {
				...
				if fc.err != nil {
					// findReplaceWorkerStep has already cancelled the shared
					// context for this error, so the cancel-aware send below
					// would race and drop the error. Deliver it with a
					// guaranteed blocking send; the consumer always drains
					// results until it is closed, so this cannot deadlock.
					results <- fc
					continue
				}
				select {

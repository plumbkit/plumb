...
//     identity decides. The name is not consulted by this predicate. (The
//     entry guards on the exported methods still refuse a claimant with no NAME
//     at all — unreachable today, since every registered session carries a
//     generated one, but it means "identity alone" describes the matching rule
//     and not the whole call path.)
...
func (s *Store) PendingNotes(ctx context.Context, who Claimant, now time.Time) ([]Row, error) {
	...
	// includeNext=false is this listing's own narrowing (see the doc comment). It
	// is the only difference in the ADDRESSING rule — the name arm this query used
	// to carry as a second, mandatory term is gone, so a note bound to this session
	// reaches the listing even when the session answers to a regenerated name. The
	// query as a whole is still not claimable minus "next": claimable also excludes
	// self-authored rows (notAuthoredBy), which this one does not.

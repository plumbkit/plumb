...
func TestReservations_KeepsClaimsItCannotShowAreOneConversation(t *testing.T) {
	...
	for _, tc := range []struct{ proxy, sess, name, ext string }{
		{"proxy-1", "sess-1", "gentle-mink", ""},
		{"proxy-2", "sess-2", "gentle-mink", ""},
		{"proxy-3", "sess-3", "linked-lark", "conv-a"},
		{"proxy-4", "sess-4", "linked-lark", "conv-a"},
	} {
...
func TestReservations_NewestWinsRegardlessOfInsertOrder(t *testing.T) {
	for _, order := range [][2]string{{"p-a", "p-b"}, {"p-b", "p-a"}} {
		...
		recordIntact(t, s, "p-a", "sess-p-a", "tame-lark", "conv-a")
		recordIntact(t, s, "p-b", "sess-p-b", "tame-lark", "conv-a")
	}
	...
	recordIntact(t, s, "p-a", "sess-p-a", "tied-lark", "conv-a")
	recordIntact(t, s, "p-b", "sess-p-b", "tied-lark", "conv-a")
}

...
func TestScoreFailedCallIsZero(t *testing.T) {
	if got := Score("read_file", "claude-desktop", 4000, 0, false); got.Total() != 0 {
		t.Errorf("failed call scored %+v, want zero", got)
	}
}

func TestScoreUnknownToolIsZero(t *testing.T) {
	if got := Score("edit_file", "claude-desktop", 4000, 0, true); got.Total() != 0 {
...
func TestScoreCapabilityGatedRead(t *testing.T) {
	...
	thin := Score("read_file", "claude-desktop", 350, 0, true)
	if thin.Capability != 100 || thin.Efficiency != 0 {
		t.Errorf("thin client read_file = %+v, want capability=100 efficiency=0", thin)
	}
	// Capable client reads files natively: a whole-file read saves nothing until the
	// ranged-read baseline lands in P3.
	capable := Score("read_file", "claude-code", 350, 0, true)
	if capable.Total() != 0 {
		t.Errorf("capable client read_file = %+v, want zero (efficiency delta deferred to P3)", capable)
	}
}
...
func TestScoreSemanticClampsWhenOutputExceedsReconstruct(t *testing.T) {
	...
	if got := Score("get_definition", "claude-code", 100000, 0, true); got.Total() != 0 {
		t.Errorf("oversized get_definition = %+v, want zero", got)
	}
}

func TestScoreSearchCapabilityGated(t *testing.T) {
	thin := Score("search_in_files", "claude-desktop", 350, 0, true)
	if thin.Capability != 100 {
		t.Errorf("thin client search = %+v, want capability=100", thin)
	}
	if got := Score("search_in_files", "claude-code", 350, 0, true); got.Total() != 0 {
...
func TestScoreBatchAvoidsPerCallOverhead(t *testing.T) {
	...
	got := Score("read_multiple_files", "claude-code", 4000, 5, true)
	if got.Efficiency != perCallOverhead*4 || got.Capability != 0 {
		t.Errorf("batch of 5 = %+v, want efficiency=%d", got, perCallOverhead*4)
	}
	if single := Score("read_multiple_files", "claude-code", 4000, 1, true); single.Total() != 0 {

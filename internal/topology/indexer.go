...
func (idx *Indexer) runQueueCycle(initial indexOp) bool {
	...
	for _, o := range ops {
		...
		// and a parent cancel would otherwise abort the whole drain. If a
		// cancellable parent is ever threaded through here (and into
		// processResync below), recordFileError must first distinguish
		// errors.Is(err, context.Canceled) — the worker is stopping and the
		// file did not earn an error row — from context.DeadlineExceeded,
		// which it did; otherwise every in-flight file is recorded as an error
		// on shutdown.

...
func allExtractors() []topology.Extractor {
	return []topology.Extractor{
		...
		ts.NewRuby(), ts.NewC(), ts.NewElixir(), ts.NewCSharp(), ts.NewJSON(), ts.NewCSS(), ts.NewPHP(), ts.NewSCSS(), ts.NewXML(), ts.NewLua(), ts.NewCpp(), ts.NewObjC(), ts.NewDart(), ts.NewRust(), ts.NewZig(), ts.NewKotlin(),

...
func selftestTierA() []string {
	return []string{
		...
		"- **Memory (read):** " + toolList(selftestMemoryRead) + ". `list_memories` first, then",
		"  `read_memory` one of the names it returns and `search_memories` for a term from that",
		"  memory's body; `relevant_memories` against a file you read above. A workspace with no",
		"  memories yet is a PASS — note the empty result rather than SKIP.",

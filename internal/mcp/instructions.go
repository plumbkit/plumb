...
You may not be working alone: session_start lists other agents active on this workspace, and workspace_sessions shows the operations they ran through plumb's write-capable tools, as the daemon recorded them (a record of calls, so a refused write and a read-only git query both appear). Before editing a file a peer recently touched, re-read it. Messages from peer agents arrive appended to your tool results; act on them when they appear, and reply with leave_note.`

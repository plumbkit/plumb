...
See the measured, reproducible numbers behind this: [**docs/use-cases.md**](docs/use-cases.md) — reading one function is 2.5×–29.6× less context than the whole file (the ratio is how much of the file you didn't need), and `find_references` returns the real call sites where a text search is 59% noise. The page publishes the losses too: `read_multiple_files` costs 1.8× **more** payload than reading the files natively. Every figure is regenerated by [`scripts/measure-use-cases.py`](scripts/measure-use-cases.py).
...
Real-binary validation has been exercised on **macOS and Linux** — as of 2026-08-21, eight of the nine adapters (Go, Python, Java, Rust, Swift, TypeScript/JS, Zig, HTML) pass their integration tests against real server binaries on Linux too; only Kotlin remains macOS-validated, because JetBrains ship `kotlin-lsp` as a download rather than through a package manager. Details, including three toolchain traps that look like adapter bugs, are in [docs/adding-an-lsp.md](docs/adding-an-lsp.md#validation-levels). Windows is [tracked but not yet supported](https://github.com/plumbkit/plumb/issues/8) — the daemon's Unix-socket architecture needs a port.

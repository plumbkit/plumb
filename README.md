...
See it run: [`docs/demos/`](docs/demos/) — `two-agents-one-file.sh` (a stale write is refused, nothing is lost) and `daemon-respawn.sh` (the daemon is killed mid-session; the agent's next edit still succeeds).

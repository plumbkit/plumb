...
- **Peer awareness** (on by default) — `workspace_sessions` names every active session and the operations it ran through plumb's write-capable tools, as the daemon recorded them. Recorded activity, not another agent's say-so: an agent about to start a task can see that a peer is already in those files. (It is a record of *calls*, so a refused write still appears — treat it as "a peer touched this", not "this landed".)

...
- **Peer awareness** (on by default) — `workspace_sessions` names every active session and the operations it ran through plumb's write-capable tools, as the daemon recorded them. Recorded activity, not another agent's say-so: an agent about to start a task can see that a peer is already in those files. (It is a record of *calls* — a refused write, or a dry-run `find_replace`, still appears — so read it as "a peer touched this", not "this landed".)
...
| **Validated** | **Java** (jdtls), **Rust** (rust-analyzer), **Swift** (sourcekit-lsp), **TypeScript/JS** (typescript-language-server), **Zig** (zls), **Kotlin** (kotlin-lsp) | Full LSP; just put the server on `$PATH` and it activates automatically |

...
`plumb serve` is a frame-aware reconnecting proxy (`internal/cli/serve_proxy*.go`) that survives a daemon crash or hang without the client noticing. On a daemon failure it keeps the client's stdio open, dial-or-spawns a fresh daemon, and **replays the captured MCP handshake** (the client only sends `initialize` once). In-flight requests get a synthesised retryable error (`code -32000`) instead of hanging; non-idempotent writes are never auto-replayed. A *hung* daemon is caught by an idle `ping` heartbeat, then `SIGTERM`→`SIGKILL`'d and respawned. Reconnects are bounded. The replayed handshake also carries a stable per-proxy session ID (in the `initialize` params' `_meta`) so the fresh daemon recognises the reconnected connection as a continuation and rehydrates its persisted per-connection state (see `[session]` `persist_state`) — making the restart transparent rather than merely non-fatal.

The session-identity half of that transparency is resolved from the **durable identity record** keyed by that same proxy session ID (`internal/sessionstate/identity.go`), and the daemon states the result back in the `initialize` RESULT's `_meta` (`dev.plumbkit/session-identity`, plus a per-process `dev.plumbkit/daemon-instance` marker). Three consequences follow, and each replaces a gate that had nothing to do with identity:

- Recovery is driven by the record alone, so it does not need the proxy to replay an ID, does not need a `session_start`, and does not need a workspace argument. A long-lived `plumb serve` predating the replay channel still comes back as itself.
- The proxy learns its own identity on the handshake rather than from a tool result, so it holds one before the first tool call and re-learns it on every reconnect.
- Because the daemon reports an *outcome* (`established` / `restored` / `degraded` / `unavailable`) and a process marker, the reconnect note can state what happened instead of asserting a restart and a wholesale state loss it has not established.

A refused recovery — a predecessor still detaching, a genuinely different live owner — leaves the record untouched and runs under a temporary identity, so a momentary overlap cannot become a permanent identity fork. Knobs: `PLUMB_PROXY_RECONNECT` (default on; off ⇒ legacy `io.Copy` proxy), `PLUMB_PROXY_HEARTBEAT` (`0` disables hang detection), `plumb serve --no-reconnect`.
...
**Multi-language discovery (two contributors).** For a `LanguageNone` root the daemon builds the workspace's language set from **two** sources, and they compose rather than excluding one another: **child markers** name the languages that carry a manifest, and a **census of the unclaimed remainder** names the ones that do not. Historically each stood down whenever the other had answered — the census (then only the last-resort sniff) ran solely when child discovery found nothing — so a single marker-carrying child hid every markerless language in the workspace: a repo with `app/tsconfig.json` and forty `.py` files in sibling directories, with no `pyproject.toml` anywhere, resolved TypeScript alone and Python appeared in no identity line, no badge, no `workspace_symbols` fan-out and no `run_task` reachability, even though per-file routing was serving those files all along.

*Child-marker discovery.* Detection only walks *up*. A `.plumb/` root that carries no language marker of its own — a Ghostty-style monorepo where the languages live one level down (`core/build.zig` + `app/Package.swift` under a bare `.plumb/` root) — would otherwise resolve as `LanguageNone` with nothing attached. So on a `LanguageNone` attach the daemon additionally descends up to `[workspace] child_scan_depth` levels (default 2) for **strong** language root markers in subdirectories (`discoverChildLanguages`, `pool_detect.go`; prunes dotdirs/`node_modules`/build outputs, stops at a matched root, never scans `$HOME`). Each discovered child language attaches its own server **rooted at the subdirectory** (`core/` for zig, `app/` for swift) via the existing multi-LSP-per-root machinery; one is elected the connection **primary** (go-first, then alphabetical), the rest attach lazily on first file. All discovered languages are listed in the `session_start` identity line (e.g. `Language: Swift, Zig`) and `workspace_sessions`, and `workspace_symbols` **fans out** across them.

*Markerless census.* After child discovery, plumb censuses the part of the root **no discovered marker claims** (`censusMarkerlessLanguages`, `pool_detect_census.go`) and nominates every **active** language holding a material share of it. The claimed subtrees are pruned from the walk — a server already covers them, and counting them would let the claimed tree decide the remainder's answer — through a `skipPath` predicate that prunes by absolute path where the existing `skipDir` prunes by base name. A language a marker has already named is skipped: a manifest is a declaration, a file count only an inference. A seat requires **both** at least 5 source files **and** at least 10% of the unclaimed remainder; a floor alone would admit eight `.py` helpers in a 30k-file TypeScript monorepo, a share alone would let a single `.py` file qualify on the strength of the directory it sits in. The share's denominator counts only languages that could themselves be nominated **and are not already claimed** — `langsupport` recognises `json`, `yaml` and `markdown`, none of which has a language server, and a language that already has one is not part of the remainder that decides whether to start another. Both exclusions are load-bearing: counting non-servable types let a 400-file JSON fixture tree veto a real nomination, and counting the claimed language put a Go repo's 40-file `tools/` Python tree at 9% of 440 files and nominated nothing at all. Under this denominator the absolute floor does more of the work — 8 `.py` helpers in a 30k-file TypeScript monorepo are 100% of the *unserved* code and qualify on the floor alone — while the share still decides between several unserved languages. The walk is bounded at 4 levels and 5000 files (its own constants, so tuning it never moves the last-resort sniff), prunes the same noise dirs — including `testdata`, `fixtures` and `third_party`, whose manifests are test assets rather than projects to serve — and honours `.gitignore`/`.ignore`. Nominated languages are rooted at the **workspace root** — the same root per-file routing resolves for those files, so discovery and routing share one server rather than starting two — and are marked *sniffed*. The census is independent of `child_scan_depth`, which asks a different question.

*A root that has its own language discovers its siblings too* (`[workspace] discover_siblings`, default on; `siblingLanguages`, `conn_attach_siblings.go`). Both contributors run beneath a marker-carrying root as well, so a `go.mod` repo with `web/tsconfig.json` and a manifest-less `tools/` Python tree reports all three rather than Go alone. This was the mirror of the markerless gap: discovery stopped as soon as the root's own marker answered, so the siblings reached no identity line, no badge, no fan-out and no `run_task` reachability, while per-file routing served them lazily throughout.

The root's own language stays the **primary** — `electPrimary` is deliberately not called here, since re-electing could hand the primary to a sibling and silently swap the server of every workspace that gains one on upgrade. Siblings are surfaced, fan out and route; they do not compete. A child root whose language IS the primary is dropped, its files being covered by the root's own server by containment, and the root's own language is put back at the head of the displayed set, which describes the whole workspace rather than "what else is here". A root with no siblings keeps a nil discovered set, so a single-language repo keeps the primary-only symbol path and its one-word label. Turning the knob off restores the older single-language attach, buying back a child walk and a census on every attach.

*Election is two-tier.* A **marker-backed** root always outranks a sniffed one when the primary is elected. Within the **sniffed** tier the census's file count decides, falling back to the usual go-first-then-alphabetical order only on an equal count; within the marker-backed tier that language order applies as before, tie-broken by root path (`lessDiscovered`). Ordering the sniffed tier alphabetically instead — the first version of this — elected `html` for a markerless repo of 100 `.py` files and 30 `.html` templates, which is both the failure `weakLangAt` records as already fixed once and a silent change to what a markerless root attaches, since that path was previously `extLangAt` and `extLangAt` picks the dominant language. So a censused language is surfaced, fans out and routes, but never displaces a primary a marker chose: without this tier a sniffed `python` would outrank a marker-backed `typescript` purely alphabetically and silently swap the server of every workspace it joined. When neither contributor finds anything, the last-resort content sniff in step 3 above still applies, **unthresholded**, so a small single-language markerless repo keeps attaching as it always has.

**Strong vs weak root markers.** Promiscuous markers — `package.json` (typescript), `index.html` (html), `requirements.txt`/`uv.lock` (python) — are **weak** (`weak_root_markers`): they name the language only of the directory they sit in directly (the resolution dir, or a `.git/`/`.plumb/` boundary), **never** an ancestor. So a stray tooling `package.json` up the tree (a docs build, or a global `~/package.json`) cannot hijack a Go/Swift/Rust workspace as TypeScript; a real JS/TS project — `package.json` at its own root or `.git` boundary — still resolves. Strong markers always beat weak ones at the same directory. The ancestor walks are additionally bounded at `$HOME`: a stray marker in the home directory never captures a workspace beneath it.

**Contested markers (either kind).** Several languages can claim the same directory: `build.gradle.kts` is a strong marker for both java and kotlin, and an ordinary frontend app ships `index.html` beside `package.json`. Language order alone cannot settle that — it is go-first-then-alphabetical, so java would win every Kotlin Gradle project and html every app with a landing page. A contested directory is instead decided by what the project *contains*: the tied language owning the most source files beneath it (`resolveMarkerTie`, shared by `strongLangAt` and `weakLangAt`). The markers that created the tie cast no vote, because a build script or an `index.html` is itself a source file of one of the languages contesting it. The scan runs only on a tie, and falls back to language order when it cannot separate the candidates. The two paths differ in one respect: when the scan hits its file cap, the **strong** path discards the partial count and falls back to language order (every candidate brought a build file, so no ordering of peers is systematically wrong), while the **weak** path reads the partial count, because there the order is a standing bias towards markup rather than a neutral choice.
...
Cold-start resolution in `session_start`: the daemon's already-attached root → explicit `workspace` arg → `roots/list` query → the serve workspace pre-pin (`--workspace`/`PLUMB_WORKSPACE`) → otherwise an honest "pass `workspace`" error. There is **no daemon-side `os.Getwd()` fallback** (the daemon is a singleton shared across connections), and **no serve-cwd fallback either** — cwd is not intent (an MCP client spawns `serve` from its own launcher's directory), so the proxy transports only an explicit pre-pin in the initialize `_meta` (`dev.plumbkit/workspace`): always validated through `Detect` (marker required, `$HOME` excluded) and never persisted as the sticky pin. A `serve` started without `--workspace`/`PLUMB_WORKSPACE` starts **unattached** — no workspace, no pin — and `session_start({workspace})` is then the sole pin authority; a client that reports no folder (Claude Desktop sends no `roots`) pins the project with an absolute `workspace`, and a relative-path call fails closed until it does. Run `plumb init` to create a `.plumb/` marker.
...
5. the serve workspace pre-pin above (`--workspace`/`PLUMB_WORKSPACE`; a serve started without either stays unattached here), then first-tool-call path seeding.
...
**Forcing the primary language.** `session_start` also takes an optional `language` arg (the `[lsp.<lang>]` key) that forces the primary language server when detection cannot infer it — now rarely needed for Xcode since `*.xcodeproj`/`*.xcworkspace` are root markers; still useful when even those are absent (e.g. a loose `.swift` directory), where the workspace would otherwise be `LanguageNone` (per-file routing still attaches secondaries, but `workspace_symbols` and the hierarchies need a *primary*). It re-pins the current workspace (or pins alongside an explicit `workspace`) to that server and shows it in the identity line. The language must be active (installed + enabled); an unknown, uninstalled or disabled value is **refused**, naming which of those it is and the remedy for that case, and leaves both the pin and the primary untouched. It used to be ignored, with detection's answer kept silently — which meant an agent on a workspace that had misdetected could correct it explicitly, be answered with the original wrong language, and have nothing to read that distinguished an honoured override from a discarded one.

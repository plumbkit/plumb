...
These scripts are the source for any demo GIF. The README's
`docs/assets/daemon-respawn.gif` is generated by one target (run from the repo
root — it records headlessly, no visible terminal needed):
...
[agg](https://github.com/asciinema/agg) (`brew install agg`, or
`asciinema-agg-bin` from the AUR). Rerun it after changing the demo script and
commit the refreshed GIF. Under the hood it is just
`asciinema rec --window-size 96x28 --command=<script>` piped into `agg`
(see the Makefile); [`vhs`](https://github.com/charmbracelet/vhs) works too if
you prefer a `.tape` file.

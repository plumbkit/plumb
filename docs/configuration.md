...
| `exclude_patterns` | []string | `[]` | Extra path globs the indexer and the file watcher skip, **on top of** the hardcoded skip list (`vendor`, `node_modules`, `testdata`, `dist`, `build`, `__pycache__`, every dot-directory) and the tree's own `.gitignore` / `.ignore`. Since those cover generated and ignored trees already, this exists for the case they cannot express: a vendored or generated tree the repository **tracks on purpose**. Globs use the same syntax as `find_files` — a pattern containing `/` matches the workspace-relative path (`third_party/**`), one without matches a base name at any depth (`*.pb.go`), and `**` spans path components. A pattern that matches **every** path is refused — the write fails, naming the pattern — rather than honoured, because it would empty the index while every query still reported success; use `enabled = false` to turn the index off. The check is a probe through the matcher itself, not a list of spellings, so `**`, `**/**`, `**/**/*` and `?*` are all caught, as is a pattern made of wildcards alone (`*/**`) that names no tree at all. **Gated on `plumb trust`**: a pattern keeps named files out of the index while the rest of it stays healthy, so a cloned repository could hide a file from `topology_search`, `topology_explore`, `topology_affected` and `workspace_search` — every one of which would then answer successfully about a tree with a hole in it. Your global value stands until you approve the project's. |
...
`persist_state` (default on; env `PLUMB_PERSIST_SESSION_STATE`) makes a **daemon restart transparent to a connected agent**. Everything is written to `session_state.db` (in the data dir, beside `stats.db`) and keyed by a stable proxy session ID that `plumb serve` generates for itself, injects into the `initialize` handshake `_meta`, and replays on every reconnect. That ID is a 122-bit secret plumb never writes to a session file, a log line, or any tool result — presenting it is evidence of being the same serve process, which is what makes the record an authorisation rather than a hint.
...
| `mailbox` | bool | `true` | **Gated on `plumb trust`.** Agent-to-agent messaging between sessions **on this workspace**: `leave_note`, `check_messages`, message delivery at `check_messages`/`session_start` with a non-consuming preview on other tool results, and unread listing in `workspace_sessions`. On by default — same-project agents are working on shared code and the bodies never leave the project. |
| `cross_project` | bool | `false` | **Gated on `plumb trust`.** Opt-in: also **receive** messages from sessions pinned to a *different* workspace. Deliberately the recipient's decision, not the sender's — a session reads the daemon-level cross-project store only when its own project sets this, so another project can never inject text into this one's context uninvited. `leave_note` checks this before sending: an un-opted-in recipient refuses the send up front, naming the reason, rather than accepting a message that would otherwise sit unclaimed until it expires. It is also the **display** consent the TUI and web dashboards read: a cross-project thread appears in their daemon-wide "Cross-Project Conversations" panel only when *every* workspace in it has set this, and what appears is volume alone (opaque thread id, note and unread counts, last activity) — never a body, a session name, or a workspace path. |
...
| `root_markers` | []string | Files whose presence identifies a workspace of this language. A marker containing `*` is a glob (`*.xcodeproj`); weak markers may be globs too (`*.py.lock`). |
...
|---|---|---|
| `go` | `gopls` | `go.mod` |
| `python` | `pyright-langserver --stdio` | `pyproject.toml`, `setup.py`, `pyrightconfig.json` (weak: `requirements.txt`, `uv.lock`, `*.py.lock`) |
...
persist_state             = true            # persist read-tracking + pinned workspace + session identity across a daemon restart (env PLUMB_PERSIST_SESSION_STATE)
persist_state_ttl_minutes = 1440            # how long the EXPENDABLE state (reads, pin) lingers before pruning; 0 disables pruning. The identity record is retained regardless of age.
...
weak_root_markers = ["requirements.txt", "uv.lock", "*.py.lock"]

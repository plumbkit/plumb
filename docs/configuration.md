...
| `mailbox` | bool | `true` | **Gated on `plumb trust`.** Agent-to-agent notes between sessions **on this workspace**: `leave_note`, `check_messages`, at-most-once atomic claims across tool results and `session_start` (not end-to-end exactly-once), plus pending/sent delivery state and conversation volume in `workspace_sessions`. On by default. |
...
| `chat_budget_bytes` | int | `4096` | UTF-8-safe byte cap on a durably stored and delivered note body. Sender and recipient both receive exact visible/total/missing counts plus a follow-up/file-path remedy; no cut is silent. `0` uses the default. |

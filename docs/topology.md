...
- **Topology is the Map.** Use it for discovery: "where is the routing logic?",
  ...
  Ruby, C, PHP, JSON, CSS, SCSS, JavaScript, Rust, Zig, Kotlin, Java, Bash, HCL, SQL, Dockerfile, TOML,
...
has no extractor for — C++, Objective-C, XML, C#, Elixir, Lua, Scala
and Dart — the files are still recorded, but they
...
1. **Extractors read your source.** Per language, a lightweight extractor turns
   ...
   `go/parser` + `go/ast` (precise, no cgo); Python, Ruby, C, PHP, JSON, CSS, SCSS, Rust, Zig, Kotlin, Java,

...
- **Topology is the Map.** Use it for discovery: "where is the routing logic?",
  ...
  Ruby, C, JavaScript, Rust, Zig, Kotlin, Java, Bash, HCL, SQL, Dockerfile, TOML,
...
has no extractor for — C++, Objective-C, JSON, XML, C#, PHP, Elixir, Lua,
...
1. **Extractors read your source.** Per language, a lightweight extractor turns
   ...
   `go/parser` + `go/ast` (precise, no cgo); Python, Ruby, C, Rust, Zig, Kotlin, Java,

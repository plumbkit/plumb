...
| `plumb setup kimi-code` | `$KIMI_CODE_HOME/mcp.json` (or `~/.kimi-code/mcp.json`; `mcpServers` key — the legacy Kimi Desktop reads the same file; Kimi Work does NOT, register it with `kimi-work`) + `$KIMI_CODE_HOME/skills/` (skills via `plumb skills sync`) |
| `plumb setup kimi-code --lean` | Same, plus an `enabledTools` allowlist pinning plumb's lean tool set |
| `plumb setup kimi-work` | The Kimi Work desktop app's bundled kernel config: `~/Library/Application Support/kimi-desktop/daimon-share/daimon/runtime/kimi-code/home/mcp.json` on macOS (`mcpServers` key; `KIMI_WORK_HOME` overrides the kernel home, and is the only route on other platforms — the app's data layout there is unverified). The app does not read `~/.kimi-code/mcp.json`, so the `kimi-code` registration does not cover it. No `--lean` (`enabledTools` is unverified against the app); skills need no separate channel — the app's kernel reads the CLI's `~/.kimi-code/skills`, which `plumb skills sync kimi-code` already populates |
